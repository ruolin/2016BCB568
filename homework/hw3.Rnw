...
likelihood = function(para, verbose = F){
  ...
  L_x0_right = 1;
  for( n in 2: n_states){
    L_x0_left = L_x0_left + expm(Q*0.0497)[x_0, n] * L_x1[n];
    L_x0_right = L_x0_right + expm(Q*0.0592)[x_0, n] * L_x2[n];
  }
  L_x0 = L_x0_left * L_x0_right;
  L_x0
}

suppressWarnings( optim(c(0.1, 0.2 , 3), likelihood, control=list(fnscale = -1)) )
